...
<div class="min-h-screen p-4 sm:p-6 lg:p-8 relative font-franklin" style="background: linear-gradient(165deg, #FFFEF7 0%, #f0f7f0 50%, #FEFDFB 100%);">
    ...
    <div class="relative z-10 bg-warm-white rounded-xl sm:rounded-2xl overflow-hidden border border-clsu-green/10 max-w-[1100px] mx-auto"
        ...
        <div class="relative px-4 sm:px-6 lg:px-8 py-4 sm:py-6"
             style="background: linear-gradient(135deg, {{ $isReschedule ? '#3b82f6 0%, #2563eb' : '#006633 0%, #004d26' }} 100%);">
            {{-- Decorative circle --}}
            ...
            <h1 class="relative z-10 font-playfair font-bold text-xl sm:text-2xl text-white mb-1 sm:mb-2 flex items-center gap-2 sm:gap-3">
                <i class="fa fa-video{{ $isReschedule ? '' : '-camera' }} text-clsu-yellow"></i>
                {{ $isReschedule ? 'Reschedule Virtual Orientation' : 'Virtual Orientation Schedule' }}
            </h1>
            <p class="relative z-10 text-white/90 text-sm sm:text-base">
                {{ $isReschedule ? 'Select a new time slot for the virtual orientation' : 'Schedule a virtual orientation session for selected applicants' }}
            </p>
        </div>

        <form action="{{ $isReschedule ? route('orientation.updateReschedule', $schedule->id) : route('orientation.store') }}" method="POST">
            @csrf
            ...
            <div class="p-4 sm:p-6 lg:p-8">
                ...
                <div class="mb-5 sm:mb-7">
                    ...
                    <select name="orientation_platform" 
                        ...
                        <option value="Zoom" {{ ($schedule->orientation_platform ?? old('orientation_platform')) == 'Zoom' ? 'selected' : '' }}>Zoom</option>
                        <option value="Google Meet" {{ ($schedule->orientation_platform ?? old('orientation_platform')) == 'Google Meet' ? 'selected' : '' }}>Google Meet</option>
                        <option value="Microsoft Teams" {{ ($schedule->orientation_platform ?? old('orientation_platform')) == 'Microsoft Teams' ? 'selected' : '' }}>Microsoft Teams</option>
                        <option value="Other" {{ ($schedule->orientation_platform ?? old('orientation_platform')) == 'Other' ? 'selected' : '' }}>Other</option>
                    </select>
                </div>
                ...
                <div class="mb-5 sm:mb-7">
                    ...
                    <input type="url" 
                           ...
                           required 
                           value="{{ $schedule->meeting_link ?? old('meeting_link') }}">
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5 sm:mb-7">
                    <div>
                        ...
                        <input type="text" 
                               ...
                               value="{{ $schedule->meeting_id ?? old('meeting_id') }}">
                    </div>
                    <div>
                        <label class="block font-semibold text-gray-800 mb-2 text-sm sm:text-base">
                        ...
                        <input type="text" 
                               ...
                               value="{{ $schedule->meeting_password ?? old('meeting_password') }}">
                    </div>
                </div>
                ...
                <div class="mb-5 sm:mb-7">
                    ...
                    <textarea name="notes" 
                              ...
                              rows="3"
                              class="w-full px-4 sm:px-5 py-2.5 sm:py-3 border-2 border-clsu-green/10 rounded-xl bg-white transition-all duration-300 text-gray-800 text-sm sm:text-base focus:outline-none focus:border-clsu-green focus:ring-4 focus:ring-clsu-green/10" 
                              placeholder="Any additional information for the attendees...">{{ $schedule->notes ?? old('notes') }}</textarea>
                </div>
                                        ...
                                        <td class="px-3 sm:px-4 py-2.5 sm:py-3 border-b border-r border-clsu-green/10 text-center last:border-r-0">
                                            <div class="flex justify-center items-center">
                                                <input type="radio" 
                                                       ...
                                                       @if($isReschedule && $schedule->orientation_date == $date->toDateString() && $schedule->orientation_time == $time) checked @endif
                                                       required>
                                            </div>
                                        </td>
                                    @endforeach
                ...
                <div class="flex flex-col sm:flex-row justify-end gap-3 sm:gap-4 pt-4 border-t border-clsu-green/10">
                    ...
                    <button type="submit" 
                            ...
                            style="background: linear-gradient(135deg, {{ $isReschedule ? '#3b82f6 0%, #2563eb' : '#006633 0%, #004d26' }} 100%); box-shadow: 0 4px 15px {{ $isReschedule ? 'rgba(59, 130, 246, 0.3)' : 'rgba(0, 102, 51, 0.3)' }};">
                        <i class="fa fa-{{ $isReschedule ? 'refresh' : 'paper-plane' }}"></i>
                        {{ $isReschedule ? 'Confirm Reschedule' : 'Send Orientation Invite' }}
                    </button>
                </div>
            </div>
        </form>
        ...
        <div class="flex justify-between items-center px-4 sm:px-6 lg:px-8 py-2.5 sm:py-3"
             style="background: linear-gradient(90deg, {{ $isReschedule ? '#3b82f6 0%, #2563eb' : '#006633 0%, #004d26' }} 100%);">
